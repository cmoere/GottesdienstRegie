...
assert.equal(builds.length,69);assert.deepEqual(builds.filter(build=>build.current).map(build=>build.version),[require('../package.json').version]);assert.equal(builds.filter(build=>build.version!=='0.36.13').every(build=>!!build.overview?.text?.de),true);
app.whenReady().then(async()=>{const win=new BrowserWindow({show:false,webPreferences:{offscreen:true}});try{
 ...
 assert.ok(result.current.includes('0.41.0'));assert.ok(result.current.includes('Sprachen'));assert.ok(result.previous.includes('0.40.0'));assert.ok(result.old.includes('Einordnung dieser Version'));
 console.log('PASS: release manifest and 69 rendered releases with language packs, rich notes and loop placement rules');win.destroy();app.exit(0);
