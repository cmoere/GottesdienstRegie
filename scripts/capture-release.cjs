...
app.whenReady().then(async()=>{
  const window=new BrowserWindow({width:1200,height:760,show:false,backgroundColor:'#eef3f4',webPreferences:{contextIsolation:true,nodeIntegration:false}});
  await window.loadFile(path.join(__dirname,'..','dist','index.html'),{hash:'history'});
  ...
  ].map((entry,index)=>({id:`demo-${index}`,presentationId,revisionId:`rev-${148-index}`,timestamp:new Date(now.getTime()-index*240000).toISOString(),actor:index===3?'Josias':'Corbin',userId:index===3?'josias':'corbin',userDisplayName:index===3?'Josias':'Corbin',action:entry[0],actionType:entry[2],category:entry[1],entityType:entry[1]==='SERVICEZEIT'?'serviceTime':'serviceItem',entityId:`entity-${index}`,entityTitle:entry[3],before:entry[4],after:entry[5],source:'desktop',transactionId:`tx-${index}`,syncStatus:index===0?'PENDING':'SYNCED'}));
  ...
  await window.webContents.executeJavaScript(`localStorage.setItem('gottesdienstregie2.presentation',${JSON.stringify(JSON.stringify({state,version:13}))});location.reload()`);
  await new Promise(resolve=>setTimeout(resolve,1800));
  const image=await window.webContents.capturePage();
  await fs.writeFile(path.join(__dirname,'..','public','help','release-0.35.0.png'),image.toPNG());
  app.quit();
});
