...
const languageSource=fs.readFileSync('src/languageCatalog.ts','utf8'),pickerSource=fs.readFileSync('src/LanguagePicker.tsx','utf8');assert.equal(/[\u{1F1E6}-\u{1F1FF}\u{1F300}-\u{1FAFF}]/u.test(languageSource+pickerSource),false);const flagSource=fs.readFileSync('src/flags/flagCatalog.ts','utf8'),flagNames=[...flagSource.matchAll(/assets\/([a-z]+\.svg)/g)].map(match=>match[1]);assert.ok(flagNames.includes('globe.svg'));for(const name of new Set(flagNames)){const asset=fs.readFileSync(path.join('src/flags/assets',name),'utf8');assert.ok(asset.includes('<svg'),name)}
