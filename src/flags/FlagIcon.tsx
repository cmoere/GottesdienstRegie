...
export function FlagIcon({languageCode,decorative=false}:{languageCode:string;decorative?:boolean}){const[src,setSrc]=useState(()=>flagAssetForLanguage(languageCode));return <img className="language-flag" src={src} onError={()=>setSrc(fallback)} alt={decorative?'':`Sprachsymbol ${languageCode.toUpperCase()}`} aria-hidden={decorative||undefined}/>}
