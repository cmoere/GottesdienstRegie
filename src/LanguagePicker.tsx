...
export function LanguagePicker({ value, onChange, source = 'en', disabled = false }: { value: string; onChange: (code: string) => void; source?: string; disabled?: boolean }) {
  ...
  return <div className="language-picker">
    <button type="button" disabled={disabled} onClick={() => setOpen(value => !value)}><span className="language-code">{selected.code.toUpperCase()}</span> {selected.name} <span className="material-symbols-outlined" aria-hidden="true">arrow_drop_down</span></button>
