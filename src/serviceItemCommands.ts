...
export function slideSvgDataUrl(slide:Slide){
  ...
  const elements=[...slide.elements].filter(element=>element.visible).sort((a,b)=>a.zIndex-b.zIndex).map(element=>{
    ...
    if(element.type==='shape'){const kind=String(p.shapeKind??'rectangle'),fill=xml(p.fill??'#68aab5'),stroke=xml(p.stroke??'none'),strokeWidth=Number(p.strokeWidth??0),common=`fill="${fill}" stroke="${stroke}" stroke-width="${strokeWidth}" opacity="${element.opacity}"`;if(kind==='ellipse')return `<ellipse cx="${element.x+element.width/2}" cy="${element.y+element.height/2}" rx="${element.width/2}" ry="${element.height/2}" ${common}/>`;const points=kind==='triangle'?`${element.x+element.width/2},${element.y} ${element.x+element.width},${element.y+element.height} ${element.x},${element.y+element.height}`:kind==='diamond'?`${element.x+element.width/2},${element.y} ${element.x+element.width},${element.y+element.height/2} ${element.x+element.width/2},${element.y+element.height} ${element.x},${element.y+element.height/2}`:kind==='arrow'?`${element.x},${element.y+element.height*.32} ${element.x+element.width*.62},${element.y+element.height*.32} ${element.x+element.width*.62},${element.y+element.height*.08} ${element.x+element.width},${element.y+element.height*.5} ${element.x+element.width*.62},${element.y+element.height*.92} ${element.x+element.width*.62},${element.y+element.height*.68} ${element.x},${element.y+element.height*.68}`:'';if(points)return `<polygon points="${points}" ${common}/>`;return `<rect x="${element.x}" y="${element.y}" width="${element.width}" height="${element.height}" rx="${kind==='rounded'?Number(p.radius??42):0}" ${common}/>`}
