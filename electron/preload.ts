...
contextBridge.exposeInMainWorld('desktop', {
  ...
  serviceContext:{open:(position:{x:number;y:number},entries:unknown[],theme?:'system'|'light'|'dark')=>ipcRenderer.invoke('service-context:open',position,entries,theme),onCommand:(callback:(id:string)=>void)=>{const listener=(_event:Electron.IpcRendererEvent,id:string)=>callback(id);ipcRenderer.on('service-context:command',listener);return()=>ipcRenderer.removeListener('service-context:command',listener)}},
  ...
  media:{list:()=>ipcRenderer.invoke('media:list'),import:(kind?:'audio'|'video')=>ipcRenderer.invoke('media:import',kind),update:(id:string,patch:unknown)=>ipcRenderer.invoke('media:update',id,patch),remove:(id:string)=>ipcRenderer.invoke('media:remove',id),onlineStatus:()=>ipcRenderer.invoke('media:online-status'),onlineList:()=>ipcRenderer.invoke('media:online-list'),cloudList:()=>ipcRenderer.invoke('media:cloud-list'),unsplashSearch:(query:string,accessKey:string,page?:number)=>ipcRenderer.invoke('media:unsplash-search',query,accessKey,page),legacyList:()=>ipcRenderer.invoke('media:legacy-list'),sync:(id:string)=>ipcRenderer.invoke('media:sync',id),cloudRemove:(id:string)=>ipcRenderer.invoke('media:cloud-remove',id)},
  mediaWindow:{open:(context:'manage'|'select',purpose:'item'|'background'|'foreground'|'audio',targetType?:'section'|'serviceItem',targetId?:string)=>ipcRenderer.invoke('media-window:open',context,purpose,targetType,targetId),close:()=>ipcRenderer.invoke('media-window:close'),select:(asset:unknown,purpose:string)=>ipcRenderer.invoke('media-window:select',asset,purpose),selectAudio:(assets:unknown[],targetType:string,targetId:string)=>ipcRenderer.invoke('media-window:select-audio',assets,targetType,targetId),onSelected:(callback:(payload:unknown)=>void)=>{const listener=(_event:Electron.IpcRendererEvent,payload:unknown)=>callback(payload);ipcRenderer.on('media:selected',listener);return()=>ipcRenderer.removeListener('media:selected',listener)},onContext:(callback:(payload:unknown)=>void)=>{const listener=(_event:Electron.IpcRendererEvent,payload:unknown)=>callback(payload);ipcRenderer.on('media-window:context',listener);return()=>ipcRenderer.removeListener('media-window:context',listener)}},
  historyWindow:{open:()=>ipcRenderer.invoke('history-window:open'),close:()=>ipcRenderer.invoke('history-window:close')},
  session: {
