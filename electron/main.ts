...
app.whenReady().then(async() => {
  ...
  ipcMain.handle('media:unsplash-search',async(_event,query:string,accessKey:string)=>{const bundledAccessKey='RHfoQj6tMXFaNTUL4AjWNJBcegsRvMqySRd-kp1hrTg',resolvedKey=accessKey?.trim()||process.env.UNSPLASH_ACCESS_KEY?.trim()||bundledAccessKey;const url=new URL('https://api.unsplash.com/search/photos');url.searchParams.set('per_page','30');url.searchParams.set('content_filter','high');url.searchParams.set('query',query?.trim()||'church worship');const response=await fetch(url,{headers:{authorization:`Client-ID ${resolvedKey}`,'accept-version':'v1','user-agent':'PGB-Present/GottesdienstRegie'},signal:AbortSignal.timeout(15000)});if(!response.ok){const detail=await response.text().catch(()=>'');throw new Error(`UNSPLASH_HTTP_${response.status}:${detail.slice(0,120)}`)}const data=await response.json() as {results?:Array<any>};return(data.results??[]).map(photo=>({id:`unsplash-${photo.id}`,name:photo.alt_description||photo.description||`Foto von ${photo.user?.name??'Unsplash'}`,path:photo.links?.html??'',kind:'image',size:0,checksum:photo.id,downloadUrl:photo.urls?.regular??photo.urls?.small,updatedAt:photo.updated_at,tags:[`Foto: ${photo.user?.name??'Unsplash'}`],visibility:'community'}))});
