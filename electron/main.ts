import { app, BrowserWindow, ipcMain, screen, safeStorage, Menu, shell, dialog, protocol, net, clipboard, nativeImage, type MenuItemConstructorOptions } from 'electron';
...
app.whenReady().then(async() => {
  ...
  ipcMain.handle('service-context:open',(event,position:{x:number;y:number},entries:Array<{id?:string;label?:string;type?:'normal'|'separator';enabled?:boolean;accelerator?:string;submenu?:any[]}>)=>{const owner=BrowserWindow.fromWebContents(event.sender);if(!owner)return false;const build=(items:typeof entries):MenuItemConstructorOptions[]=>items.map(entry=>entry.type==='separator'?{type:'separator'}:{label:String(entry.label??''),enabled:entry.enabled!==false,accelerator:entry.accelerator,submenu:entry.submenu?build(entry.submenu):undefined,click:()=>event.sender.send('service-context:command',entry.id)});Menu.buildFromTemplate(build(entries)).popup({window:owner,x:Math.round(position.x),y:Math.round(position.y)});return true});
  ...
  ipcMain.handle('media:unsplash-search',async(_event,query:string,accessKey:string)=>{if(!accessKey?.trim())throw new Error('UNSPLASH_ACCESS_KEY_REQUIRED');const url=new URL('https://api.unsplash.com/search/photos');url.searchParams.set('per_page','30');url.searchParams.set('query',query?.trim()||'church worship');const response=await fetch(url,{headers:{authorization:`Client-ID ${accessKey.trim()}`,'accept-version':'v1','user-agent':'GottesdienstRegie-MediaBrowser'},signal:AbortSignal.timeout(15000)});if(!response.ok)throw new Error(`UNSPLASH_HTTP_${response.status}`);const data=await response.json() as {results?:Array<any>};return(data.results??[]).map(photo=>({id:`unsplash-${photo.id}`,name:photo.alt_description||photo.description||`Foto von ${photo.user?.name??'Unsplash'}`,path:photo.links?.html??'',kind:'image',size:0,checksum:photo.id,downloadUrl:photo.urls?.regular??photo.urls?.small,updatedAt:photo.updated_at,tags:[`Foto: ${photo.user?.name??'Unsplash'}`],visibility:'community'}))});
