...
app.whenReady().then(async() => {
  ...
  ipcMain.handle('media:cloud-list',async()=>{const [remote,local]=await Promise.all([onlineMedia.list(),mediaRepository.list()]);return remote.map(item=>{const match=local.find(asset=>asset.github?.path===item.path||asset.checksum===item.checksum);return{...item,id:match?.id??item.id,name:match?.name??item.name.replace(/^[a-f0-9]{40,64}-/i,'').replace(/\.[^.]+$/,''),favorite:match?.favorite??false,tags:match?.tags??[],aiGenerated:match?.aiGenerated,lastUsedAt:match?.lastUsedAt,createdAt:match?.createdAt,updatedAt:match?.updatedAt??item.updatedAt,extension:item.path.split('.').at(-1)?.toUpperCase()??'',visibility:'team'}})});
