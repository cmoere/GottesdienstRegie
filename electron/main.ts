...
app.whenReady().then(async() => {
  ...
  ipcMain.handle('media:mark-used',(_event,id:string)=>mediaRepository.markUsed(id));
  ...
  ipcMain.handle('media:save-generated',(_event,input:{name:string;kind:'image'|'video';extension:string;data:Uint8Array;tags?:string[]})=>mediaRepository.importGenerated({...input,data:Buffer.from(input.data)}));
  ...
  ipcMain.handle('media:cloud-list',async()=>{const [remote,local]=await Promise.all([onlineMedia.list(),mediaRepository.list()]),remoteItems=remote.map(item=>{const match=local.find(asset=>asset.github?.path===item.path||asset.checksum===item.checksum);return{...item,id:match?.id??item.id,name:match?.name??item.name.replace(/^[a-f0-9]{40,64}-/i,'').replace(/\.[^.]+$/,''),favorite:match?.favorite??false,tags:match?.tags??[],aiGenerated:match?.aiGenerated,lastUsedAt:match?.lastUsedAt,createdAt:match?.createdAt,updatedAt:match?.updatedAt??item.updatedAt,extension:item.path.split('.').at(-1)?.toUpperCase()??'',visibility:'team'}}),localOnly=local.filter(asset=>!asset.github).map(asset=>({id:asset.id,name:asset.name,path:asset.fileName,kind:asset.kind,size:asset.size,checksum:asset.checksum,downloadUrl:asset.url,updatedAt:asset.updatedAt,createdAt:asset.createdAt,lastUsedAt:asset.lastUsedAt,favorite:asset.favorite,tags:asset.tags,aiGenerated:asset.aiGenerated,extension:asset.extension,visibility:'private'}));return[...localOnly,...remoteItems]});
  ipcMain.handle('media:unsplash-search',async(_event,query:string,accessKey:string,page=1)=>{const bundledAccessKey='RHfoQj6tMXFaNTUL4AjWNJBcegsRvMqySRd-kp1hrTg',resolvedKey=accessKey?.trim()||process.env.UNSPLASH_ACCESS_KEY?.trim()||bundledAccessKey;const url=new URL('https://api.unsplash.com/search/photos');url.searchParams.set('per_page','30');url.searchParams.set('page',String(Math.max(1,Math.floor(page))));url.searchParams.set('content_filter','high');url.searchParams.set('query',query?.trim()||'church worship');const response=await fetch(url,{headers:{authorization:`Client-ID ${resolvedKey}`,'accept-version':'v1','user-agent':'PGB-Present/GottesdienstRegie'},signal:AbortSignal.timeout(15000)});if(!response.ok){const detail=await response.text().catch(()=>'');throw new Error(`UNSPLASH_HTTP_${response.status}:${detail.slice(0,120)}`)}const [data,local]=await Promise.all([response.json() as Promise<{results?:Array<any>}>,mediaRepository.list()]);return(data.results??[]).map(photo=>{const match=local.find(asset=>asset.checksum===photo.id||asset.github?.path===photo.links?.html);return{id:`unsplash-${photo.id}`,name:photo.alt_description||photo.description||`Foto von ${photo.user?.name??'Unsplash'}`,path:photo.links?.html??'',kind:'image',size:0,checksum:photo.id,downloadUrl:photo.urls?.regular??photo.urls?.small,updatedAt:photo.updated_at,tags:match?.tags?.length?match.tags:[`Foto: ${photo.user?.name??'Unsplash'}`],favorite:match?.favorite??false,visibility:'community'}})});
